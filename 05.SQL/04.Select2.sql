...
select publisher, count(*), avg(price) from book
    group by publisher
    having count(*) >= 2;
-- 가격이 8,000원 이상인 도서를 구매한 고객에 대해여 고객별 주문 도서의 총 수량 및 평균 금액
...
select * from orders, customer
    where orders.custid = custoner.custid;
...
select o.orderid, o.saleprice, o.orderdate, b.bookname, c.name
    ...
    where o.custid = custid and o.bookid = b.bookid;
...
select c.custid, c.name, o.saleprice
    from customer c
    left join orders o          -- left outer join에서 outer 생략가능
    on c.custid = o.custid;
